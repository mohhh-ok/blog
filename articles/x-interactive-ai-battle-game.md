...
![観客参加のループ。Xのいいね・リプライがゲーム世界に反映され、誕生や取組結果がXに投稿される](https://raw.githubusercontent.com/mohhh-ok/blog/main/src/content/posts/2026/06-10-kassen-x-loop.svg)
...
![リプライの多段防御。untrustedなリプライ原文が、隔離呼び出し・越境点の限定・データブロック・出口フィルタを経てX投稿に至る](https://raw.githubusercontent.com/mohhh-ok/blog/main/src/content/posts/2026/06-10-kassen-injection-defense.svg)
...
- `events` テーブルに tick ごとの「起きたこと全部」（集計結果・LLM 生成物そのもの・ターンログ・投稿内容）を1レコードで追記。乱数シードは最初の genesis イベントに記録し、各 tick の乱数はそこから決定的に導出する
- 状態テーブル（妖怪・job・ゲージ…）はすべて投影（projection）で、いつでも再構築できる
- 1 tick = 1 トランザクション。途中失敗（使用上限エラー等）なら何も書かれない — 偽 tick が残らず、巻き戻し処理自体が不要
- `bun run rollback --to-tick N` で任意 tick へロールバック。再構築は LLM 不要・課金ゼロ（生成物はイベントに記録済み＋エンジンは決定的）で、数千 tick でも数秒

「LLM の出力をイベントに記録する」ことで、非決定な LLM を含むシステム全体の再現性を担保しているのがミソです。

![イベントソーシングによる永続化。追記専用のeventsテーブルから状態テーブルを投影し、任意tickへロールバックできる](https://raw.githubusercontent.com/mohhh-ok/blog/main/src/content/posts/2026/06-10-kassen-event-sourcing.svg)
