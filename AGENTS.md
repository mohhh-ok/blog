...
冒頭は「こんにちは、フリーランスエンジニアのmohです。」で始める。
一般的な口調を用いる（AGENT.mdより）。
未来の日付でも問題なし。

#### 読む人の視点で構成する（2026-09決定）

調べた事実を並べるだけの記事にしない。読む人が「自分の場合はどうなのか」を知りたくて来ている前提で組む。

- 節の並びは、読む人が順に抱く疑問の順にする。見出しそのものは疑問形にしない（下の「見出しは節の役割を短い名詞句で」）
- 各節は「読む人の問い → 答え → だからあなたはこうなる（こうする）」の順で閉じる。事実を書いたら、それが読む人に何を意味するかを必ず一文添える
- 結論は最初に置き、「怖がる場所」と「手を打てば済む場所」のように読む人の判断単位で分ける。項目は 1〜2 文に詰め、根拠は本文に送る
- 規約や仕様の原文引用は、読む人の判断に効く一文だけ残し、残りはリンクにする。引用の羅列で節を埋めない
- 「じゃあ何をすればいいのか」に相当する節を必ず置き、効く順に並べる
- 実例や表は、規約より先に見せたほうが伝わる場合は先に出す。表の件数と本文の件数は必ず一致させる

判定の目安: 節の並びが、読む人の疑問が順に解けていく流れになっているか。なっていなければ情報の押し付けなので組み直す。

#### 見出しは節の役割を短い名詞句で（2026-09決定）

- 見出しは「その節に何が書いてあるか」の案内。結論、探索の手順、使い分け、気をつける点、のような名詞句で 10 字前後
- 見出しに結論・主張・仕組みの説明を書かない。書き手が仕掛けを言いたくて書いた見出し（「X には Y を渡し、3 段で降りる」）は自己満足で、読む人の役に立たない。結論は節の最初の 1 文に書く
- 疑問形にしない。「〜のか」の見出しは、読む人がまだ抱いていない問いを書き手が押し付ける形で、並ぶと単調で見下した調子になる
- 判定: 見出しだけ抜き出して、記事の目次として自然に見えるか

#### 分かりにくいと言われたら、短くする（2026-09決定）

「詳しく書けば分かりやすくなる」は誤り。分かりにくさの原因は語の多さと構造の多さで、説明を足すと悪化する。

- 直すときは、文と語を減らす。段落を増やす・手順を細かく割る・例を足す方向の修正は禁止
- 定義が要る語（経路・段・ビーム等）は、その語を使わずに書けないかを先に試す
- LLM を人の役割語（書き手・担当者・筆者等）で呼ばない。人が居るように読める。LLM・モデル・Jev のように、記事で最初に使った呼び名をそのまま使う
- 具体例は 1 つだけ。既に図や JSON が置いてあるなら、本文はそれを指すだけにする
- 直した後の節は、直す前より短くなっていなければやり直し

#### コードは載せず、概念で書く（2026-09決定）

読む人はコードを読みに来ていない。実装は AI に頼む時代なので、記事に必要なのは「何を渡して、何を聞いて、何が返るか」と「結果をどう読むか」で、呼び出しの書き方ではない。

- 実装コードは blog-examples に置き、記事からはリンクだけ張る
- 本文では処理の流れを番号付きの文で書く。関数名・引数・SDK の呼び出し形は書かない
- 載せてよいのは、読む人がそのまま入力する 1 行（インストールコマンド等）と、設定値や定義そのもの（性格文・プロンプト等）が短い場合。後者もコードブロックより箇条書きを優先する
- API に送る JSON と返る JSON のサンプルは 1 組載せてよい。実走した実物を使い、値は本文の数字と一致させる
- 数値の結果は表にし、本文で引く数字は表と一致させる
...
### SVG は必ずサイト背景色を明示する（2026-08決定）

**SVG を作るときは必ず最初に `<rect width="..." height="..." fill="#f2f0ee"/>` を入れて、サイト背景色 `#f2f0ee` (= `--gray-light`, rgb(242,240,238)) をキャンバス全面に敷く。** 透明背景のまま出すと、ブラウザで拡大表示したときに OS のダークモードやビューアの背景色に負けて図の線・文字が読めなくなる。透明を許容しない。

- `#f2f0ee` を背景に敷く関係上、**淡い塗りのボックスに `#f2f0ee` を使わない**（背景と同化して消える）。ボックス塗りは `#ffffff` を使う
- viewBox の幅・高さと `<rect>` の width/height は一致させる（部分敷きにしない）
- 既存 SVG を更新するときも先頭 rect の有無を毎回確認
...
- ファイル名＝スラッグ（半角英数字・ハイフン・アンダースコア、12〜50字）。**`YYYY-MM-` プレフィックスの 8 字も 50 字に含まれる**ので、残りの説明部分は 42 字以内。作成時に `echo -n "<ファイル名から .md を除いたもの>" | wc -c` で数えてから commit する（超えると Zenn のデプロイが中断され、記事は作られない。2026-09 に 54 字で中断した）
...
- **新規記事は 24 時間に 1 本まで**（2026-09 実測）。Zenn は直近 24 時間の新規投稿数（予約投稿も含む）に非公開の上限を設けていて、超えると「投稿数の上限に達したためデプロイされませんでした」で弾かれる。前日に 1 本出していたら、その push から 24 時間経つまで新しい `articles/*.md` を `published: true` で push しない。弾かれた記事は次のデプロイ（何かを push したとき）で再判定されるだけで自動再試行は無いので、時間が過ぎたら空コミット（`git commit --allow-empty -m "zenn: retry"`）を push する。既存記事の更新は数に入らない
