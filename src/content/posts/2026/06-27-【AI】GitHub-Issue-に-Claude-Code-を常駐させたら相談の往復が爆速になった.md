...
### 4. ユーザの `~/.claude` 設定を読ませない

`CLAUDE_CONFIG_DIR` を tmpdir に固定して、グローバル `CLAUDE.md` や `settings.json` を一切読ませないようにしています。`alwaysThinkingEnabled` のような重い設定が紛れ込んで思考トークンが暴走するのを防ぐためです。
