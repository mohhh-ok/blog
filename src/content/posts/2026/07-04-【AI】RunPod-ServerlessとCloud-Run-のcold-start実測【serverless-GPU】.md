...
- **RunPod SDXL cold は 4 回中 3 回が 12s 台、1 回だけ 61s**。ばらつきが大きい
- **Cloud Run cold は再現性が高い**: n=3〜4 とも ±5s
- **中央値では RunPod のほうが速い**が、**最悪ケースは Cloud Run のほうが安定**
...
## Finding 2: RunPod Whisper は狭い範囲

n=4 とも 2.49-4.72s、max/min = 1.9x。SDXL のような跳ねは観測されず。
...
±5s 前後で毎回同水準。「Cloud Run SDXL cold は 45s あれば入る」と言い切れる程度に安定。**SLO を切りやすい**。

## Finding 4: 中央値 vs 最悪ケース
...
**中央値では RunPod のほうが速い**。ただし **SDXL の最悪ケースは Cloud Run のほうが速い** (61s vs 40s)。ユーザ体感の p50 なら RunPod、p95/p99 の安定は Cloud Run。
...
|3GB 以下のモデル | RunPod (Whisper 側の狭い分散)|
