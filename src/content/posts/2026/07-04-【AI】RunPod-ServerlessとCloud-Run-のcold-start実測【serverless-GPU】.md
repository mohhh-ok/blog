...
「RunPod のほうが cold start 早いって噂だけど本当か」を確かめるべく、**同一 workload** で **RunPod Serverless (Network Volume 構成)** と **Cloud Run GPU** を実測しました。
...
「serverless GPU なら RunPod > Cloud Run」ではなく、**モデルサイズと idle パターン次第で逆転する**。10GB クラスの diffusion / LLM モデルを間欠アクセスで使う用途では Cloud Run のほうが安定するかもしれません。
...
大モデル + まちまちのアクセス間隔という条件が刺さるユースケースなら、**Cloud Run の予測しやすさが効きます**。逆に RunPod は Network Volume に weight を並べておいて別モデルに切り替えられる柔軟性が魅力で、実験的な用途で有利です。
